...
if request.QueryString("action")="UnionOrder" then 
    if thedingdan<>"" and thedingdan<>yundantxm then
        ...
        if not(rs.bof and rs.eof) then
            sql="update fenxiao_uniondingdan set WayBill_UnionOrder='"&yundantxm&"' where WayBill_UnionOrder='"&thedingdan&"'"
            conn.execute(sql)
            response.write "<script language=javascript>alert('合并订单成功！');location.href='Distibution_AddWayBill_TXM.asp';</script>"
        else
             sql="select id from daili_uniondingdan where WayBill_UnionOrder='"&thedingdan&"'"
             set rs1=conn.execute(sql)
             if not(rs1.bof and rs1.eof) then
                sql="update daili_uniondingdan set WayBill_UnionOrder='"&yundantxm&"',yuandingdan='wl'+yuandingdan  where WayBill_UnionOrder='"&thedingdan&"'"
                conn.execute(sql)
                response.write "<script language=javascript>alert('合并订单成功！');location.href='Distibution_AddWayBill_TXM.asp';</script>"
            else
                response.write "<script language='javascript'>alert('合并的订单条码不存在！');location.href='Distibution_AddWayBill_TXM.asp';</script>" 
            end if 
			Set rs1=Nothing 
        end If
        Set rs=Nothing 
...
set rsc=conn.execute("select distinct dingdan from Order_Distribution_Main where dingdan in (select yuandingdan from fenxiao_uniondingdan where WayBill_UnionOrder='"&yundantxm&"') ")
...
set rsc=conn.execute("select dingdan from Order_Agent_Main where dingdan in (select right(yuandingdan,16) from daili_uniondingdan where WayBill_UnionOrder='"&yundantxm&"') ")
do while not rsc.eof
...
set rsb=conn.execute("select distinct shouhuoname,usertel from fenxiao_uniondingdan where WayBill_UnionOrder='"&yundantxm&"'")
...
if (rs.bof and rs.eof) then
    ...
    if dailidingdan<>"" then 
	    ...
	    for i=0 to ubound(thedingdan)
            conn.execute("update Order_Agent_Main set wuliu_id="&wuliu_id&",yundanhao='"&yundanhao&"',is_postwuliu=1 where dingdan ='"&thedingdan(i)&"' and shopxp_shfs<>27 and shopxp_shfs<>23 ")
	    next
    end if
    if IsSend="on" then
        set rsi=conn.execute("select wy.fahuo_date,wy.yundanhao,wy.dangdiwuliu_tel,wg.wuliu_name,wy.shouhuo_tel  from wuliu_yifahuo as wy inner join wuliu_gongshi as wg on wy.wuliu_id=wg.id where yundanhao='"&yundanhao&"'")
        if not  rsi.eof then
        sms_content="您好，您的商品已通过"&rsi("wuliu_name")&"于"&rsi("fahuo_date")&"发货，运单号："&yundanhao&",详情请咨询"&rsi("dangdiwuliu_tel")&""
	        if trim(rsi("usertel"))>"" then	
...
<table width="95%" border="0" cellpadding="2" cellspacing="1" bgcolor="#DBDBDB" style="margin-top:5px; margin-left:10px">
  <form name="form1" method="post" action="../OrderPrint/?action=save" onSubmit="return checkdata()" autocomplete="off">
    ...
    <tr bgcolor="#FFFFFF" >
      ...
      <td style="PADDING-LEFT: 10px"><input name="shouhuo_tel" type="text" id="shouhuo_tel" value="<%=rsb("usertel")%>" size="50"> <input type="checkbox" id="IsSend"  name="IsSend" checked/>是否发送信息</td>
    </tr>
    <tr bgcolor="#FFFFFF" >
    ...
    <tr bgcolor="#FFFFFF" >
	  ...
	  <textarea name="dingdan" cols="70" rows="8" id="dingdan" readonly ><% if dailidingdan <>"" then %><%=left(dailidingdan,len(dailidingdan)-1)%><%end if  %></textarea>
      <a href="../OrderPrint/?act=deldingdan" >      清空订单</a></td>
    </tr>
    <tr bgcolor="#FFFFFF" >
      <td height="30" align="right">分销中心订单:</td>
      <td height="30" style="PADDING-LEFT: 10px">
      <textarea name="fenxiaodingdan" cols="70" rows="8" id="fenxiaodingdan" readonly ><%if fenxiaodingdan<>"" then %><%=left(fenxiaodingdan,len(fenxiaodingdan)-1)%><%end if  %></textarea>
      <a href="../OrderPrint/?act=delfenxiaodingdan">清空订单</a></td>
    </tr>
    <tr bgcolor="#FFFFFF" >
      <td height="30"></td>
      <td height="30" style="PADDING-LEFT: 10px">
...
<%
Call connClose	'close connection
%>
