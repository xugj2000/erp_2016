...
<%
''call GetPageUrlpower("order_cancel/dingdan_quxiao_fenxiao_list.asp")'取得父级页面的所有权限
...
id=trim(request("id"))
 sql="select dingdan,addkucun,is_fanbi,pro_type,userid,username,shopxpptname,shopxpptid,p_size,productcount,danjia,zonger,op_user,actiondate,quxiao_date,shopxp_shfs,zhifufangshi,cangku_type,tiaohuoren,jiehuoren,is_validate,quxiao_liyou,fuwuzhekou,warehouse_confirm,warehouse_confirmuser,warehouse_confirmdate from CancelOrder_Distribution where id="&id
 ...
 End if
 if shopxpptname<>"" then
 sql=sql&" and shopxpptname like '%"&shopxpptname&"%'"
 end if
 if cangku_type<>"" then
 sql=sql&" and cangku_type='"&cangku_type&"'"
 end if
'response.write sql
'response.end
...
%>
      <tr>
        <td width="29%" height="25" bgcolor="#FFFFFF">是否增加库存</td>
        <td width="71%" bgcolor="#FFFFFF"><%=Print_yes_no(rst("addkucun"))%></td>
      </tr>
      <tr>
      ...
      <tr>
        <td height="25" bgcolor="#FFFFFF">取消员</td>
