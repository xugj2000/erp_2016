<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="productinone.aspx.cs" Inherits="Warehousing.Storage.productinone" %>
...
<tr bgcolor="#FFFFFF">
  ...
  <tr bgcolor="#FFFFFF" align="center">
    ...
    <td><input name="p_quantity" type="text" class="text width4" value='<%#Convert.ToDouble(Eval("p_quantity"))%>'<%= (sm_status==1?" readonly":"")%>/></td>
	<td><input name="p_price" type="text" class="text width2" value='<%#Eval("p_price")%>'/></td>
	<td><%if (sm_status != 1)
      {%><span class="delete_btn" onclick="$(this).parent().parent().remove();"></span><%} %></td>
  </tr>
          </ItemTemplate>
          </asp:Repeater>
	</table>
    <%if (sm_status != 1)
      {%>
<a href="javascript:;" id="add_onerow" class="add_onerow" style="color:red;margin:4px;display:block">添加新的商品</a>
